...
def SimCLR_new(net, device, batch_size, train_loader, num_epoch, augment1, augment2, temp=0.5, lr=1e-4, optim='adam', lr_scheduler='None'):
    ...
    train_loss_list = []
    model_checkpoint = 'SimCLR_new_batchsize=' + str(batch_size) + '_lr=' + str(lr) + '_optim=' + optim + '_temp=' + str(
        temp) + '_epoch=' + str(num_epoch) + '.pt'
    net.train()

    for epoch in range(num_epoch):
        print('Epoch ' + str(epoch+1))
        train_loss = 0
        for i in tqdm(range(len(augment1))):
            aug1 = augment1[i]
            aug2 = augment2[i]
            actual_batch_size = aug1.shape[0]  # taking care of the last batch in case it cannot be fully divided
            loss_func = NTXent_Loss(actual_batch_size, temp)
